 ...
 * store.
 */
private[sql] class FluxStateStoreProvider extends StateStoreProvider with Logging {

  // CCCS toggle compression
  val USE_COMPRESSED_STREAMS = false

  class HDFSBackedReadStateStore(val version: Long, map: HDFSBackedStateStoreMap)
    extends ReadStateStore {

    override def id: StateStoreId = FluxStateStoreProvider.this.stateStoreId
  ...
  class HDFSBackedStateStore(val version: Long, mapToUpdate: HDFSBackedStateStoreMap)
    ...
    var FROMSTATE_TIMER_INDEX = 6
    var TOSTATE_TIMER_INDEX = 7
    val FLUXSTATE_NUM_FIELDS = 8

    // CCCS: We hard code the number of blooms
    val NUM_BLOOMS = 10

    val fluxGroupStateEnc = Encoders.product[FluxGroupState].asInstanceOf[ExpressionEncoder[FluxGroupState]]
    val fluxGroupStateSerializer = fluxGroupStateEnc.createSerializer()
    val keyEncoder = ExpressionEncoder[String]
    ...
    def getTagCount(row: UnsafeRow) = getFluxStateRow(row).getInt(TAGCOUNT_INDEX)
    ...
    override def put(key: UnsafeRow, value: UnsafeRow): Unit = {
      ...
      toStateTimer.add(System.currentTimeMillis() - getToStateTimer(valueCopy))
      fromStateTimer.add(getFromStateTimer(valueCopy))
    }
  ...
  def getMetricsForProvider(): Map[String, Long] = synchronized {
    val metrics = Map("memoryUsedBytes" -> SizeEstimator.estimate(loadedMaps),
  ...
  override def getStore(version: Long): StateStore = {
    ...
    logInfo(s"Retrieved version $version of ${FluxStateStoreProvider.this} for update")
  ...
  override def getReadStore(version: Long): ReadStateStore = {
    val newMap = getLoadedMapForStore(version)
    logInfo(s"Retrieved version $version of ${FluxStateStoreProvider.this} for readonly")
